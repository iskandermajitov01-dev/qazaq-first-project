...
const Layout = ({ children }) => {
  ...
  return (
    <div className="min-h-screen pb-20 md:pb-0 md:grid md:grid-cols-[280px_1fr_260px]">
    <aside className="hidden rounded-tr-[40px] rounded-br-[40px] border-r-2 border-brand-bean bg-brand-cream p-6 shadow-neon md:block">
        ...
        {navItems.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `${baseClass} ${
                isActive
                  ? "bg-brand-mint text-brand-bean border-2 border-brand-bean"
                  : "text-brand-bean hover:bg-brand-rose/30 border-2 border-transparent"
              }`
            }
          >
    ...
    <aside className="hidden flex-col gap-4 border-l-2 border-brand-rose bg-brand-blush/20 p-4 md:flex">
      <div className="rounded-[32px] border-2 border-brand-rose bg-white/90 p-5 text-sm text-brand-bean shadow-neon">
        ...
        <p className="text-xs text-brand-bean/80">Здесь можно разместить рекламу курсов, партнёрские предложения или новости спонсоров.</p>
        <div className="mt-4 rounded-3xl bg-brand-mint/10 p-4 text-sm font-semibold text-brand-bean">Рекламный баннер</div>
      </div>
      <div className="rounded-[32px] border-2 border-brand-rose bg-white/90 p-5 text-sm text-brand-bean shadow-neon">
        <h2 className="mb-3 text-xl font-bold text-brand-bean">Партнёр</h2>
        <p className="text-xs text-brand-bean/80">Оставь место для спонсора, конференции или новых наборов в команду.</p>
